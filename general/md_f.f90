...
!f2py intent(out) r 
      ...
      SUBROUTINE make_angle_matrix_f(r_mtx,coords,bond_max1,bond_max2,n_atoms,teta)  
      integer*8 :: n_atoms, i, j, k, axes 
!f2py intent(in) n_atoms
      real*8 :: bond_max1, bond_max2, bond_min
      real*8, dimension(n_atoms, 3)  :: coords 
...
!f2py intent(out) teta  
      ...
      do k = 1, n_atoms
        do j = 1, n_atoms
           if (r_mtx(j, k).gt.bond_min & 
                   .and. r_mtx(j, k).lt.bond_max2 ) then 
              do i = 1, n_atoms 
                 if ( r_mtx(i, j).gt.bond_min & 
                   .and. r_mtx(i, j).lt.bond_max1) then 
                    if (i.ne.j .and. i.lt.k .and. j.ne.k) then  
                       do axes = 1, 3
                          uij(axes) = coords(j, axes) - coords(i, axes)
